...
# ----------------------
  ...
  it '26. returns the names of items that are associated with one or more orders' do
    ...
    # pluck has to be at the end since its a 'calculation' which you can't really chain other ar methods on top of
  ...
  it '28. returns a table of information for all users items' do
    ...
    custom_results = User.joins(:orders, :order_items).distinct
                         .select('users.*, count(distinct order_items.id) as total_item_count')
                         .group(:id).order(:name)
    # ---------------------------------------------------------------
